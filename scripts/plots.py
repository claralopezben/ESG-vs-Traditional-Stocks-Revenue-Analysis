...
plots.py
--------
Generate every figure used in the blog post and save as PNG to plots/.

Style choices:
- Single colour palette across all figures (ESG vs Traditional contrast).
- White background, light grid, sans-serif, no top/right spines.
- Each figure stands alone: descriptive title, axis labels, legend.
...
# Style
# --------------------------------------------------------------------------
ESG_COLOUR  = "#2A9D8F"   # teal-green
TRAD_COLOUR = "#E76F51"   # warm orange
GREY        = "#6C757D"
...
def plot_cumulative_returns(returns: pd.DataFrame) -> None:
    ...
    for tkr in cum.columns:
        ax.plot(cum.index, cum[tkr],
                label=tkr,
                color=_color_for(tkr),
                lw=1.6,
                alpha=0.85)
...
# --------------------------------------------------------------------------
# Plot 2: rolling annualised volatility
# --------------------------------------------------------------------------
def plot_rolling_vol(returns: pd.DataFrame, window: int = 63) -> None:
    ...
    for tkr in rolling.columns:
        ax.plot(rolling.index, rolling[tkr],
                label=tkr, color=_color_for(tkr), lw=1.4, alpha=0.85)
...
def plot_return_distributions(returns: pd.DataFrame) -> None:
    ...
    bins = np.linspace(min(esg_basket.min(),  trad_basket.min()),
                       max(esg_basket.max(),  trad_basket.max()),
                       80)
...
# --------------------------------------------------------------------------
# Plot 5: CAPM alpha & beta bar chart
# --------------------------------------------------------------------------
